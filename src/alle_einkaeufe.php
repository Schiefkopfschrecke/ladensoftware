...
$sql = 'SELECT zeit, preis, betreff FROM buchungen_mitglieder ORDER BY zeit DESC';
$res = db_anfragen($sql);
while ($einkaufsdaten = mysqli_fetch_assoc($res)) {
	echo '<h3>' . $einkaufsdaten['betreff'] . ': ' . date("d.m.Y - H:i", strtotime($einkaufsdaten['zeit'])) . ' für CHF ' . number_format($einkaufsdaten['preis'] * (-1), 2, ".", "'") . '*</h3>';
	if ($einkaufsdaten['betreff'] = 'Einkauf Webseite') {
		echo '<table>';
		$sql = 'SELECT * FROM warenkorb WHERE eingekauft = "' . $einkaufsdaten['zeit'] . '"';
