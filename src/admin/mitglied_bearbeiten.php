...
if ($_POST) {
	...
	$sql = 'UPDATE 
				mitglieder 
			SET
				geschlecht = "' . $_POST['geschlecht'] . '", vorname = "' . $_POST['vorname'] . '", name = "' . $_POST['name'] . '", strasse ="' . $_POST['strasse'] . '", adrzusatz = "' . $_POST['adrzusatz'] . '", plz = "' . $_POST['plz'] . '", ort = "' . $_POST['ort'] . '", telefon = "' . $_POST['telefon'] . '", email = "' . $_POST['email'] . '", benutzername = "' . $_POST['benutzername'] . '", iban = "' . $_POST['iban'] . '", wbg = "' . $_POST['wbg'] . '", status = "' . $_POST['status'] . '" WHERE id = ' . $_POST['mitglied_id'];
	$res = db_anfragen($sql);
...
$mitglied_id = filter_input(INPUT_GET, 'mitglied_id', FILTER_VALIDATE_INT);
$sql = 'SELECT * FROM mitglieder WHERE id = ' . $mitglied_id;
$res = db_anfragen($sql);
$mitglieddaten = mysqli_fetch_assoc($res);
echo '<div id="kontoaenderung"><form action="" method="post">

			<input type="hidden" name="mitglied_id" id="mitglied_id" value="' . $mitglied_id . '">
			<table>
			<tr>
				<td>Guthaben:</td>
				<td>';
$sql = 'SELECT SUM(preis) AS guthaben FROM buchungen_mitglieder WHERE mitglied_id = ' . $mitglied_id;
$res = db_anfragen($sql);
...
echo '</td></tr>
			<tr>
				...
				<td>' . $mitglieddaten['id'] . '</td></tr>
			<tr>
				<td>Anzahl Anteilscheine</td>
				<td>' . $mitglieddaten['anteilscheine'] . '</td></tr>
			<tr>
				<td>Mitglieschaftsstatus:</td>
				<td><select id="status" name="status">';
foreach ($config['mitgliedstatus'] as $ein_status) {
	echo '<option value="' . $ein_status . '" ';
	if ($ein_status == $mitglieddaten['status']) {
		echo 'selected';
	}
	echo '>' . $ein_status . '</option>';
}
echo '</select></td></tr>
				<tr>
				<td>Anrede:</td>
