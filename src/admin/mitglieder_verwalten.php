...
<table>
	<tr>
		...
		<td width="300pt" align="right"><a href="buchungen_kontrollieren.php">Buchungen Monat</a></td>
		<td width="300pt" align="right"><a href="mitglieder_einzahlungen.php">Buchungen Mitglieder</a></td>
		</< /tr>
</table>
<p>
<table>
	<thead>
		<tr>
			...
			<th data-sort="string">Letzte Buchung:</th>
		</tr>
	</thead>
	<tbody id="myTable">
		...
		while ($mitglieder = mysqli_fetch_assoc($res)) {
			...
			echo '<td class="email"><a href="mailto:' . $mitglieder['email'] . '">' . $mitglieder['email'] . '</a></td>';
			...
			<td><small><?php $sql = 'SELECT * FROM buchungen_mitglieder WHERE betreff = "Einzahlung(+)" AND mitglied_id = ' . $mitglieder['id'] . ' ORDER BY zeit DESC';
						$res_letzte_buchungen = db_anfragen($sql);
						$letzte_buchungen = mysqli_fetch_assoc($res_letzte_buchungen);
						if ($letzte_buchungen['preis'] > 0) {
